...
public class HashTable<T> {
    ...
    private int getHashValue(T key) { // Key: integer or String
        int hashValue = -1;
    ...
    public boolean contains(T key) {
        int hashValue = getHashValue(key);  // get i
        return items.get(hashValue).contains(key); // exists on current row?
    }

    public Pair<Integer, Integer> getPosition(T key) {
        if (this.contains(key)) {
            int hashValue = getHashValue(key);  // get i
            return new ImmutablePair<>(hashValue, items.get(hashValue).indexOf(key)); // get a[i][j]
        }
